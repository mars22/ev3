...
class PID:
  ...
  def __init__(self, set_point, kp=1, ki=1, kd=1):
    self.set_point = set_point
    self.kd = kd
    self.ki = ki
    self.kp = kp

    self.integral = 0  # sum of all errors from start till that moment
    self.prev_error = 0

  def update(self, current_val: int):
    current_error = self.set_point - current_val
  ...
  def run_forward_odometry(self, left_motor: ev3.LargeMotor, right_motor: ev3.LargeMotor, speed: int):
    ...
    def worker(cond: threading.Event):
      ...
      while cond.is_set():
        ...
        #   left=left_motor.position, right=right_motor.position, error=error)
        # )
        print('Left i: {left_i} -- d: {left_d} -- position: {position}'.format(
          left_i=left_motor.position_i, left_d=left_motor.position_d, position=left_motor.position)
        )
        print('Right i: {left_i} -- d: {left_d} -- position: {position}'.format(
          left_i=right_motor.position_i, left_d=right_motor.position_d,
          position=right_motor.position)
        )
        l_error = speed+(-error)
        r_error = speed+(+error)
        if error > 0:
